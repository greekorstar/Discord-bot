...
async function handleSetupWarnedRole(interaction) {
  ...
  await interaction.reply({ content: `✅ **${role.name}** will be given after a member's first warning.`, ephemeral: true });
}

async function handleSetupWarnLogChannel(interaction) {
  const channel = interaction.options.getChannel('channel');
  settingsStore.set('warnLogChannelId', channel.id);
  await interaction.reply({ content: `✅ Warnings and punishment decisions will now be logged in ${channel}.`, ephemeral: true });
}

async function handleSetupPunishmentChannel(interaction) {
  const channel = interaction.options.getChannel('channel');
  settingsStore.set('punishmentChannelId', channel.id);
  await interaction.reply({ content: `✅ Punishment decisions (mute/kick/ban prompts, undo/keep ban prompts) will now be posted in ${channel}.`, ephemeral: true });
}
...
async function handleWarn(interaction) {
  ...
  const { count, summary } = await punishmentSystem.processWarning(target.id, reason, interaction.guild);

  await requestLog.logAction(
    { executorId: interaction.user.id, executorTag: interaction.user.tag, action: 'warn', targetId: target.id, targetLabel: target.tag, reason },
    interaction.guild,
    'warnLogChannelId'
  );

  await interaction.reply(`⚠️ Warned **${target.tag}** (warning #${count}). Reason: ${reason}\n${summary}`);
}
...
module.exports = {
  ...
  handleSetupWarnLogChannel,
  handleSetupPunishmentChannel,
  handleKick,
  handleBan,
  handleUnban,
  handleMute,
