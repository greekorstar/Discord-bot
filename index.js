...
client.on('interactionCreate', async interaction => {
  ...
  try {
    ...
    if (interaction.customId === 'embed_basics_modal') {
      ...
      // or one of the fixed-purpose buttons (apply_button/ticket_button) shared elsewhere.
      if (removed.customId && removed.customId !== 'apply_button' && removed.customId !== 'ticket_button') {
      ...
      if (styleInput === 'application' || styleInput === 'app') {
        ...
        const roleId = embedBuilder.parseRoleId(target);
        if (!roleId) {
          await interaction.reply({ content: `⚠️ For a role action, put the role there — either a role mention (\`@RoleName\`, paste from the role picker) or its numeric ID. "${target}" isn't either.`, ephemeral: true });
          return;
        }

        const action = ROLE_ACTIONS[styleInput];
        const customId = embedBuilder.genButtonId();
        buttonRegistry.setConfig(customId, { type: action.type, roleId, text: replyText || undefined });
        ...
        if (style === null) {
          await interaction.reply({ content: `"${styleInput}" isn't a valid style/action. Use one of: link, primary, secondary, success, danger, application, ticket, togglerole, addrole, removerole.`, ephemeral: true });
          return;
        }
        ...
        if (style === ButtonStyle.Link) {
          ...
          const customId = embedBuilder.genButtonId();
          const wantsPublic = ['public', 'visible', 'everyone'].includes((target || '').trim().toLowerCase());
          buttonRegistry.setConfig(customId, { type: 'message', text: replyText || 'This button has no reply text set.', ephemeral: !wantsPublic });
...
client.on('interactionCreate', async interaction => {
  ...
  try {
    ...
    // Default: plain message button
    await interaction.reply({ content: config.text || 'This button has no reply configured.', ephemeral: config.ephemeral !== false });
  } catch (error) {
    console.error('Error handling custom action button:', error);
    const errMsg = { content: "Something went wrong performing this button's action — I may be missing the Manage Roles permission, or my role may be positioned below the role it's trying to change.", ephemeral: true };
    if (interaction.replied || interaction.deferred) {
      await interaction.followUp(errMsg);
    } else {
      await interaction.reply(errMsg);
