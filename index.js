...
const founderSetup = require('./founderSetup.js');
const auditMonitor = require('./auditMonitor.js');
  ...
  new SlashCommandBuilder()
    ...
    .addSubcommandGroup(group =>
      group.setName('member')
        .setDescription('Actions targeting a member')
        .addSubcommand(sub =>
          sub.setName('kick')
            .setDescription('Kick a member')
            .addUserOption(o => o.setName('target').setDescription('Member to kick').setRequired(true))
            .addStringOption(o => o.setName('reason').setDescription('Explain why').setRequired(true)))
        .addSubcommand(sub =>
          sub.setName('ban')
        ...
        .addSubcommand(sub =>
          sub.setName('warn')
            .setDescription('Warn a member for misusing /request access')
    ...
    .addSubcommandGroup(group =>
      group.setName('channel')
        ...
        .addSubcommand(sub =>
          sub.setName('perm')
            .setDescription('Allow, deny, or reset a permission on a channel for a role or member')
            .addChannelOption(o => o.setName('channel').setDescription('Channel to change').setRequired(true))
            .addStringOption(o => o.setName('permission').setDescription('Permission to change').setRequired(true)
              .addChoices(...['ViewChannel', 'SendMessages', 'ManageMessages', 'Connect', 'Speak', 'ManageChannels', 'ManageRoles', 'MentionEveryone', 'AttachFiles', 'EmbedLinks'].map(p => ({ name: p, value: p }))))
            .addStringOption(o => o.setName('setting').setDescription('Allow, deny, or reset to neutral').setRequired(true)
              .addChoices({ name: 'Allow', value: 'allow' }, { name: 'Deny', value: 'deny' }, { name: 'Reset', value: 'reset' }))
            .addStringOption(o => o.setName('reason').setDescription('Explain why').setRequired(true))
            .addRoleOption(o => o.setName('target_role').setDescription('Role to change (provide this OR target_user)').setRequired(false))
            .addUserOption(o => o.setName('target_user').setDescription('Member to change (provide this OR target_role)').setRequired(false))))
    .addSubcommandGroup(group =>
      group.setName('role')
    ...
    .addSubcommandGroup(group =>
      group.setName('setup')
        .setDescription('Configure /request itself (Administrator only)')
        .addSubcommand(sub =>
          sub.setName('logchannel')
            .setDescription('Set where /request actions get logged')
            .addChannelOption(o => o.setName('channel').setDescription('Log channel').setRequired(true)))
        .addSubcommand(sub =>
          sub.setName('accessrole')
            .setDescription('Set which role can use /request\'s action commands')
            .addRoleOption(o => o.setName('role').setDescription('Access role').setRequired(true)))
        .addSubcommand(sub =>
          sub.setName('warnedrole')
            .setDescription('Set a role given after a member\'s first warning')
            .addRoleOption(o => o.setName('role').setDescription('Warned role').setRequired(true)))
        .addSubcommand(sub =>
          sub.setName('warnlogchannel')
            .setDescription('Set where warnings and punishment decisions get logged')
            .addChannelOption(o => o.setName('channel').setDescription('Log channel').setRequired(true)))
        .addSubcommand(sub =>
          sub.setName('punishmentchannel')
            .setDescription('Set where punishment decisions (mute/kick/ban, undo/keep ban) get posted')
            .addChannelOption(o => o.setName('channel').setDescription('Punishment channel').setRequired(true)))),

  new SlashCommandBuilder()
    .setName('foundersetup')
    .setDescription('Configure onboarding + safety settings (server owner only)')
    .addSubcommand(sub =>
    ...
    .addSubcommand(sub =>
      sub.setName('explicitfilter')
        .setDescription('Set the explicit media content filter')
        .addStringOption(o => o.setName('level').setDescription('Filter level').setRequired(true)
          .addChoices(
            { name: 'Disabled', value: 'disabled' },
            { name: 'Members without roles', value: 'without_roles' },
            { name: 'All members', value: 'all_members' },
          )))
    .addSubcommand(sub =>
      sub.setName('safetyalerts')
        .setDescription('Set the Discord safety alerts channel')
        .addChannelOption(o => o.setName('channel').setDescription('Safety alerts channel').setRequired(true)))
    .addSubcommand(sub =>
      sub.setName('welcomescreen')
        .setDescription('Enable/disable and edit the welcome screen')
        .addBooleanOption(o => o.setName('enabled').setDescription('Enable the welcome screen?').setRequired(true))
        .addStringOption(o => o.setName('description').setDescription('Welcome screen description').setRequired(false)))
    .addSubcommand(sub =>
      sub.setName('onboarding')
        .setDescription('Enable/disable Onboarding (existing prompts are preserved)')
        .addBooleanOption(o => o.setName('enabled').setDescription('Enable Onboarding?').setRequired(true))),
...
client.on('interactionCreate', async interaction => {
  ...
  try {
    if (commandName === 'ping') {
      ...
      if (group === 'setup') {
        if (!(await permissions.requirePermission(interaction, PermissionFlagsBits.Administrator, 'Administrator'))) return;

        if (subcommand === 'logchannel') await requestCommands.handleSetupLogChannel(interaction);
        else if (subcommand === 'accessrole') await requestCommands.handleSetupAccessRole(interaction);
        else if (subcommand === 'warnedrole') await requestCommands.handleSetupWarnedRole(interaction);
        else if (subcommand === 'warnlogchannel') await requestCommands.handleSetupWarnLogChannel(interaction);
        else if (subcommand === 'punishmentchannel') await requestCommands.handleSetupPunishmentChannel(interaction);
        return;
      }

      // Every other group is gated by the configured access role (or Administrator), not a raw Discord permission
      if (!(await requestCommands.checkAccess(interaction))) return;
      ...
      if (group === 'member') {
        ...
        else if (subcommand === 'warn') await requestCommands.handleWarn(interaction);
      } else if (group === 'channel') {
        if (subcommand === 'create') await requestCommands.handleCreateChannel(interaction);
        else if (subcommand === 'delete') await requestCommands.handleDeleteChannel(interaction);
        else if (subcommand === 'edit') await requestCommands.handleEditChannel(interaction);
        else if (subcommand === 'perm') await requestCommands.handleChannelPerm(interaction);
      } else if (group === 'role') {
        if (subcommand === 'create') await requestCommands.handleCreateRole(interaction);
        else if (subcommand === 'delete') await requestCommands.handleDeleteRole(interaction);
      }
    }

    else if (commandName === 'foundersetup') {
      ...
      const subcommand = interaction.options.getSubcommand();
      if (subcommand === 'verification') await founderSetup.handleVerification(interaction);
      else if (subcommand === 'explicitfilter') await founderSetup.handleExplicitFilter(interaction);
      else if (subcommand === 'safetyalerts') await founderSetup.handleSafetyAlerts(interaction);
      else if (subcommand === 'welcomescreen') await founderSetup.handleWelcomeScreen(interaction);
      else if (subcommand === 'onboarding') await founderSetup.handleOnboarding(interaction);
    }
...
client.on('interactionCreate', async interaction => {
  ...
  try {
    ...
    if (interaction.customId === 'embed_basics_modal') {
      ...
      if (image) {
        if (!embedBuilder.isValidHttpUrl(image)) {
          await interaction.reply({ content: `⚠️ "${image}" isn't a valid image URL (must start with http:// or https://) — everything else was saved, but the image wasn't.`, ephemeral: true });
          embedBuilder.persist();
          return;
        }
        data.image = image;
      ...
      if (iconURL && !embedBuilder.isValidHttpUrl(iconURL)) {
        await interaction.reply({ content: `⚠️ "${iconURL}" isn't a valid icon URL (must start with http:// or https://). Author wasn't changed.`, ephemeral: true });
        return;
      }
      if (url && !embedBuilder.isValidHttpUrl(url)) {
        await interaction.reply({ content: `⚠️ "${url}" isn't a valid link URL (must start with http:// or https://). Author wasn't changed.`, ephemeral: true });
        return;
      }
      ...
      if (!embedBuilder.isValidHttpUrl(thumbnail)) {
        await interaction.reply({ content: `⚠️ "${thumbnail}" isn't a valid URL (must start with http:// or https://). Thumbnail wasn't changed.`, ephemeral: true });
        return;
      }
...
// ---- Punishment ladder: decision / undo / keep button clicks ----
client.on('interactionCreate', async interaction => {
  if (!interaction.isButton()) return;
  if (!interaction.customId.startsWith('punishdecide|') && !interaction.customId.startsWith('punishundo|') && !interaction.customId.startsWith('punishkeep|')) return;

  await punishmentSystem.handleButtonInteraction(interaction);
});
...
// ---- Audit log monitoring: catches /request access-role holders bypassing the bot ----
client.on('guildAuditLogEntryCreate', (entry, guild) => {
  auditMonitor.handleAuditLogEntry(entry, guild, client);
});
