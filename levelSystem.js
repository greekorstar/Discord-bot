...
async function onLevelUp(member, newLevel, sourceChannel) {
  ...
  try {
    if (config.levelUp.useDm) {
      await member.send(text).catch(() => {}); // DMs can fail silently (closed DMs) — not worth erroring over
      return;
    }
...
// ---- Weekly / monthly reset + Highlights auto-post ----
...
async function tickHighlights(client) {
  const now = new Date();
  ...
  for (const guild of client.guilds.cache.values()) {
    ...
    if (config.highlights.weekStartIso !== currentWeekStart) {
      if (channelId) await postHighlight(client, guild, channelId, 'weekly').catch(err => console.error('[levelSystem] weekly highlight failed:', err.message));
      resetPeriodXp(guild.id, 'weeklyXp');
      config.highlights.weekStartIso = currentWeekStart;
      saveConfigs();
    }

    if (config.highlights.monthStartIso !== currentMonthStart) {
      if (channelId) await postHighlight(client, guild, channelId, 'monthly').catch(err => console.error('[levelSystem] monthly highlight failed:', err.message));
...
function init(client) {
  ...
  setInterval(() => tickHighlights(client).catch(err => console.error('[levelSystem] highlights tick failed:', err.message)), 60 * 60 * 1000).unref();
}
