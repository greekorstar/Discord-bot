...
async function postLog(guild, text) {
  const logChannelId = settingsStore.get('requestLogChannelId', '');
...
async function handleAuditLogEntry(entry, guild, client) {
  try {
    ...
    const newCount = requestWarnings.addWarning(entry.executorId);
    const revertMsg = await tryRevertAuditEntry(entry, guild);

    const warnedRoleId = settingsStore.get('requestWarnedRoleId', '');
    let accessRevoked = false;
    if (newCount === 1 && warnedRoleId) {
      await member.roles.add(warnedRoleId).catch(() => {});
    }
    if (newCount >= 2 && member.roles.cache.has(accessRoleId)) {
      await member.roles.remove(accessRoleId).catch(() => {});
      accessRevoked = true;
    }

    try {
      await member.send(`You made a server change directly (outside of /request), which isn't allowed while you hold /request access. This counts as warning #${newCount}.${accessRevoked ? ' Your /request access has been removed as a result.' : ''}`);
    } catch (err) {
    ...
    await postLog(
      guild,
      `⚠️ **Out-of-band change detected**: ${member.user.tag} performed a "${entry.action}" action outside of /request (warning #${newCount})${accessRevoked ? ' — access role removed' : ''}.\n${revertMsg}`
    );
  } catch (err) {
