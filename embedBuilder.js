...
function buildAddButtonModal() {
  return new ModalBuilder()
    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('style').setLabel('Style / Action').setPlaceholder('link/primary/secondary/success/danger/togglerole/addrole/removerole').setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(20)
      ),
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('target').setLabel('URL (link) or Role (role actions)').setStyle(TextInputStyle.Short).setRequired(false)
      ),
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('replyText').setLabel('Message shown on click (optional)').setStyle(TextInputStyle.Paragraph).setRequired(false).setMaxLength(1000)
      ),
      new ActionRowBuilder().addComponents(
...
// Resolves a channel from typed input the same way resolveRole does: a real
// mention (<#ID>), a raw ID, or an exact case-insensitive name match (with
// or without a leading #). Returns the channel, or null if nothing matches.
...
// Tries, in order: a real mention (<@&ID>), a raw ID, then an exact
// case-insensitive name match (with or without a leading @). Returns the
// Role object, or null if nothing in the guild matches.
...
function buildControlRows(session) {
  ...
  const row3 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('embed_removebutton_button').setLabel('Remove Button').setStyle(ButtonStyle.Secondary).setDisabled(session.buttons.length === 0),
  ...
  // Show the actual buttons being built too, so the ephemeral preview matches
  // what will really get sent/saved instead of only showing the editor controls.
  ...
  for (const row of message.components) {
    for (const comp of row.components) {
      if (comp.style === ButtonStyle.Link) {
        ...
        // That includes fixed-ID buttons like apply_button / ticket_button — without this,
        // they'd silently vanish the moment the message is loaded into an edit session
        // and re-saved, since the message gets rebuilt entirely from session.buttons.
